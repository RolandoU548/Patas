...
export const ShapeOptions = ({ canvas }: { canvas: Canvas | null }) => {
  ...
  const handleScaleXChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const intValue = parseInt(value, 10);
    setWidth(intValue);
    selectedObject?.set({ scaleX: intValue / selectedObject.width });
    canvas?.renderAll();
  };
  ...
  const handleScaleYChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const intValue = parseInt(value, 10);
    setHeight(intValue);
    selectedObject?.set({ scaleY: intValue / selectedObject.height });
    canvas?.renderAll();
  };
    ...
    if (selectedObject.type === "rect" || selectedObject.type === "triangle")
      return (
        <section className="fixed right-4 -translate-y-1/2 top-1/2 gap-2 flex flex-col p-2 text-left bg-primary rounded-md empty:p-0 z-30">
          <Label className="text-primary-foreground" htmlFor="shape-width">
            Ancho
          </Label>
          <Input
            value={width}
          ...
          />
          <Label className="text-primary-foreground" htmlFor="shape-height">
            Alto
          </Label>
          <Input
            value={height}
      ...
      return (
        <section className="fixed right-4 -translate-y-1/2 top-1/2 gap-2 flex flex-col p-2 text-left bg-primary rounded-md empty:p-0 z-30">
          ...
          <Input
            ...
            onChange={handleStrokeChange}
          />
          <p className="text-primary-foreground text-sm font-medium leading-none peer-disabled:cursor-not-allowed er-disabled:opacity-70 flex justify-between">
            Opacidad
            <span>{opacity}</span>
          </p>
          <Slider
